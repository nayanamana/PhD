...
class TaskManager:
    ...
    def _check_failure_status(self):
        ...
        if self.failure_status:
            ...
            if self.failure_status['ErrorType'] == 'ExceedCommandFailureLimit':
                #pass
                #NAYA - do nothing of faliure limit exceeds
    ...
    def _start_thread(self, browser, command_sequence, condition=None):
        ...
        #Luminati - Naya
        try:
            #proxy_cfg = {'http':'http://127.0.0.1:24000'}
            #proxy_support = urllib2.ProxyHandler(proxy_cfg)
            #opener = urllib2.build_opener(proxy_support)
            #urllib2.install_opener(opener)

            lum_url = "http://lumtest.com/myip.json";

            #print "1111"
            req = urllib2.Request(lum_url)
            #print "222"
            response = urllib2.urlopen(req)
            #print "333"
            result = json.load(response)
            src_country = result['country']
            src_ip = result['ip']
            #print "### RESULT:"
            #print result
            if result['asn']:
               asnum = result['asn']['asnum']
               asnorg = result['asn']['org_name']
            else:
               asnum = ""
               asnorg = ""
        ...
        self.sock.send(("INSERT INTO site_visits (visit_id, crawl_id, source_ip, source_country, asnum, asnorg, site_url) VALUES (?,?,?,?,?,?,?)",
                        (self.next_visit_id, browser.crawl_id, src_ip, src_country, asnum, asnorg, command_sequence.url)))
